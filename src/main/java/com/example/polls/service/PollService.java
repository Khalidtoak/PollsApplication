...
import com.example.polls.exception.BadRequesteException;
...
import com.example.polls.payloads.PagedResponse;
import com.example.polls.payloads.PollRequest;
import com.example.polls.payloads.PollResponse;
import com.example.polls.payloads.VoteRequest;
...
public class PollService {
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, UserPrincipal currentUser) {
        ...
        if(poll.getExpirationDateTime().isBefore(Instant.now())) {
            throw new BadRequesteException("Sorry! This Poll has already expired");
        }
        ...
        try {
            ...
            throw new BadRequesteException("Sorry! You have already cast your vote in this poll");
        }
    ...
    private void validatePageNumberAndSize(int page, int size) {
        if(page < 0) {
            throw new BadRequesteException("Page number cannot be less than zero.");
        }

        if(size > AppConstants.MAX_PAGE_SIZE) {
            throw new BadRequesteException("Page size must not be greater than " + AppConstants.MAX_PAGE_SIZE);
        }
    }
